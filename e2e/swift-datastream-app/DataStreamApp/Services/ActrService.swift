...
final class ActrService: ObservableObject {
    ...
    func runAllProbes() async {
        ...
        isRunning = true
        results = []
        logLines = ["--- Starting DataChunk probe run ---"]

        var req = Local_StartProbeRequest()
        req.probeName = "run-all"

        // Use targetType from environment or default
        let targetType = ProcessInfo.processInfo.environment["ACTR_DATASTREAMAPP_TARGET_TYPE"] ?? "actrium:DuplexStreamService:1.0.0"
...
private final class ProbeHandlerImpl: ProbeServiceHandler, @unchecked Sendable {
    ...
    func startProbe(
        ...
        do {
            targetType = try ActrType.fromStringRepr(req.targetType.isEmpty ? "actrium:DuplexStreamService:1.0.0" : req.targetType)
        } catch {
        ...
        if let count = streamEchoCount(from: req.probeName) {
            ...
            let passCount = zip(result.receivedLines, expectedLines).filter { pair in
                pair.0 == pair.1
            }.count
            let succeeded = result.succeeded && passCount == count && result.receivedLines == expectedLines
            await MainActor.run {
                emitE2EResult("\(passCount)/\(count)")
            }

            var resp = Local_StartProbeResponse()
            resp.started = succeeded
            resp.message = succeeded ? "received \(passCount)/\(count)" : "expected \(expectedLines), received \(result.receivedLines)"
            return resp
        }
