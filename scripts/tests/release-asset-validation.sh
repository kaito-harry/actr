...
require(
    "Staged Maven primary is missing" in kotlin_publish,
    "Kotlin Maven reconciliation must reject missing staged primary artifacts",
)

reconcile_match = re.search(
    r"(?ms)^          reconcile_maven_primary\(\) \{\n.*?^          \}\n",
    kotlin_publish,
)
require(reconcile_match is not None, "missing Kotlin Maven primary reconciliation function")
reconcile_function = textwrap.dedent(reconcile_match.group(0))
with tempfile.TemporaryDirectory() as temporary_directory:
    behavior_script = f"""
    set -u
    probe_dir={temporary_directory!r}
    fetch_calls=0
    fetch_maven_file() {{
      fetch_calls=$((fetch_calls + 1))
      maven_http_status=200
      printf remote > "$2"
    }}
    upload_maven_file() {{ return 0; }}
    wait_for_maven_file() {{ return 0; }}
    validate_maven_pom() {{ return 0; }}
    reconcile_maven_checksums() {{ return 0; }}

    {reconcile_function}

    staged_file="kotlin-maven/io/actrium/actr/0.4.11-rc.8/actr-0.4.11-rc.8-sources.jar"
    mkdir -p "$(dirname "$staged_file")"
    printf staged > "$staged_file"
    if ! reconcile_maven_primary "$staged_file"; then
      echo "valid staging failed to reconcile with existing remote" >&2
      exit 1
    fi
    if [[ "$fetch_calls" -ne 1 ]]; then
      echo "remote-existing fixture was not exercised" >&2
      exit 1
    fi

    rm "$staged_file"
    if reconcile_maven_primary "$staged_file"; then
      echo "missing staging unexpectedly reconciled from remote" >&2
      exit 1
    fi
    if [[ "$fetch_calls" -ne 1 ]]; then
      echo "remote was consulted before staging was validated" >&2
      exit 1
    fi
    """
    behavior = subprocess.run(
        ["bash", "-c", textwrap.dedent(behavior_script)],
        text=True,
        capture_output=True,
        check=False,
    ...
    require(
        ...
        "remote-existing, staging-missing Kotlin reconciliation must fail: "
        f"{behavior.stdout}{behavior.stderr}",
    )
require(
...
require(
    kotlin_publish.count('"maven_sources_sha256"') == 2,
    "Kotlin immutable tag metadata must create and validate the sources JAR checksum",
)
