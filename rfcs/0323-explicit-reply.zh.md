...
- 日期（Date）：2026-07-08
- 修订（Revised）：2026-07-24（按 #289 review 重写，对齐 #337/#381 之后的 main）
...
admit_incoming(envelope) ──同步、按到达序──→ ACL / dedup / 键提取 / 调度器提交
        │
        └→ IncomingContinuation（执行 handler body）─┬→ handler 返回 = dispatch 完成 → 释放键槽 / budget C（on_complete）
                                                     ├→ 返回值即 response bytes（RETURN）→ 构造 RESPONSE envelope 发回
                                                     └→ mailbox ack
```
...
**deadline 语义**：`deadline()` = arrival + timeout_ms，是**调用方真实 deadline 的上界**（不含请求在途传输时间 / transit time，真实 deadline 更早）；TELL 请求返回 None。回复走与今日内联回复相同的回程：main 上所有 RESPONSE 均固定走 RpcReliable lane、按 request_id 关联，与请求进入的 lane（RpcReliable vs RpcSignal）无关；延迟回复不改变这一行为。若未来需要回复保留请求 lane，属新增行为，须另行提案并改造各 send_response 路径。与 #257（typed `Context::call` 的可配置 timeout）对齐：#257 落地后，此 timeout_ms 应取其配置值。
...
- 分期：
  ...
  - Phase 3：WIT guest ABI 扩展。运行时现仅支持单一 world——wit-v2 0.2.0（`dispatch` 已是 `async func`、可多调用并发 in-flight，但**仍返回 bytes**）；退役的 wit-v1（sync, @0.1.0）在加载时即被 `probe_world` 拒绝，仅作为浏览器 web-abi 的生成源保留（#381）。需新增 `send-reply` host import，让 `dispatch` 返回 `replied(bytes) | deferred` 变体。在此之前 WASM guest 仅支持 RETURN 模式，codegen 对 wasm 目标遇 EXPLICIT 选项报错而非静默降级（reject-don't-degrade 策略不变）。
