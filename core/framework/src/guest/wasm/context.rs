...
//! Since M3 the world is lifted synchronously (no `async: true`), so the
//! generated host imports are ordinary (non-`async`) functions that return
//! their result directly. The public [`Context`] trait stays `async` for the
//! user-facing API; these methods simply resolve the host call inline. Any
//! host-side suspension (waiting on the host to service a call) happens in the
//! wasmtime fiber on the host, transparent to this guest code.
